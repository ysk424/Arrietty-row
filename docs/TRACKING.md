...
The rider approved restarting development from accepted audio main (`c5279d2`)
after verifying the restored build. This change addresses exercise-time
occlusion of the configured handle Tracker by the rider's body. After a live
trial without stopping/stuttering, the rider adopted this as the normal
training version and requested push. Actual occlusion assistance remains
unmeasured. There is no change to device selection, BLE
...
Normal `tools/run.ps1` launches overwrite `logs/training.log`, with no automatic
