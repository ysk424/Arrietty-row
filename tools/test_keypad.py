...
def main():
    ...
    log = repo / 'logs' / f'keypad-{time.time_ns()}.log'
    ...
    process = subprocess.Popen([
        ...
        str(repo / 'unreal/ArriettyRow/ArriettyRow.uproject'),
        '/Game/Row/Maps/BledRow', '-game', '-RowOffline', '-nohmd',
        '-nosplash', '-nosound', '-windowed', '-ResX=1000', '-ResY=700',
        '-RowQuitAfter=100', f'-abslog={log}',
    ...
    def press(label, key, scan, extended, action):
        ...
        wait_until(lambda: read_log().count('ROW_CONTROL action=') >= len(expected))
        time.sleep(.3)
        ...
        assert read_log().count('route=preprocessor') == len(expected), 'Input bypassed keypad router'
    ...
    print(f'PASS 9 Windows input actions and clean shutdown. Evidence: {log}')
