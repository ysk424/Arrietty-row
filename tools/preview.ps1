[CmdletBinding()]
param([switch]$Chase,[switch]$Setup,[string]$EngineRoot='C:/Program Files/Epic Games/UE_5.8')
$ErrorActionPreference='Stop'
$repo=Split-Path -Parent $PSScriptRoot
$project=Join-Path $repo 'unreal/ArriettyRow/ArriettyRow.uproject'
$name=if($Setup){'row-setup'}elseif($Chase){'row-wake'}else{'row-view'}
...
$args=@(('"'+$project+'"'),'/Game/Row/Maps/BledRow','-game','-RowDemo','-nohmd','-RenderOffscreen',
    ...
    ('-RowScreenshotAt='+$(if($Setup){'10.5'}else{'24'})),('-RowQuitAfter='+$(if($Setup){'12'}else{'28'})),('-RowScreenshotPath="'+$output+'"'),('-abslog="'+$log+'"'),
