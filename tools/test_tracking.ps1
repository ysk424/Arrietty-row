...
$log=Join-Path $repo "logs/tracking-runtime-$stamp.log"
$image=Join-Path $repo "artifacts/tracking-$stamp.png"
$began=Get-Date
$arguments=@(('"'+$project+'"'),'/Game/Row/Maps/BledRow','-game','-RowDemo','-RowDemoStraight',
    '-RowDemoOcclusion','-nohmd','-RenderOffscreen','-unattended','-nosplash','-nosound',
...
$session=Get-ChildItem -LiteralPath (Join-Path $repo 'unreal/ArriettyRow/Saved/Sessions') -Filter '*.csv' |
